...
import type { InvestorViewOtherPostsQuery } from 'types/graphql'
...
export const QUERY = gql`
  query InvestorViewOtherPostsQuery($id: Int!) {
    investorViewOtherPosts: postsByPosterID(id: $id) {
      id
      title
      createdAt
      imageURL
...
export const Success = ({
  investorViewOtherPosts,
}: CellSuccessProps<InvestorViewOtherPostsQuery>) => {
  return (
    <div id="PostsTab" className={ProfilePageClassName}>
      {investorViewOtherPosts.map((post) => {
        return (
          <button
            key={post.id}
            onClick={() => {
