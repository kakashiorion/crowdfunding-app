...
import type { StartupMyProfilePostsQuery } from 'types/graphql'
...
export const QUERY = gql`
  query StartupMyProfilePostsQuery {
    startupMyProfilePosts: myPosts {
      id
      title
      createdAt
      imageURL
...
export const Success = ({
  startupMyProfilePosts,
}: CellSuccessProps<StartupMyProfilePostsQuery>) => {
  return (
    <div id="PostsTab" className={ProfilePageClassName}>
      {startupMyProfilePosts.map((post) => {
        return (
          <button
            key={post.id}
            onClick={() => {
